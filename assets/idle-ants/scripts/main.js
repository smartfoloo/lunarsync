...
'use strict'; {
    ...
    const HANDLER_CLASS = class VideoDOMHandler extends self.DOMElementHandler {
        ...
        TryPlayMedia(mediaElem) {
            ...
            if (this._isRuntimeInWorker && this._supportsRvfc && mediaElem["c3_rvfcId"] === -1) mediaElem["c3_rvfcId"] = mediaElem["requestVideoFrameCallback"](() => this._OnVideoFrameCallback(mediaElem))
        }
        RemovePendingPlay(mediaElem) {
        ...
        async _OnVideoFrameCallback(mediaElem) {
            if (this._destroyedVideos.has(mediaElem)) return;
        ...
        CreateElement(elementId, e) {
            ...
            if (autoplay === 0) {
                ...
                elem.autoplay = false;
                elem.preload = "auto"
            } else elem.autoplay = true;
            if (e["src"]) elem.src = e["src"];
            if (autoplay === 2) this.TryPlayMedia(elem);
            this._numVideos++;
            this._StartTicking();
            return elem
        }
        _SendPlaybackEvent(elem, elementId, num) {
            if (this._destroyedVideos.has(elem)) return;
        ...
        DestroyElement(elem) {
            ...
            if (IsVideoPlaying(elem)) elem.pause();
            elem.src = "";
            this._numVideos = Math.max(this._numVideos - 1, 0);
            if (this._numVideos === 0) this._StopTicking();
        ...
        Tick() {
            ...
            for (const [id, elem] of this._elementMap.entries()) {
                ...
                if (this._isRuntimeInWorker)
                    ...
                    } else promises.push(createImageBitmap(elem).then(imageBitmap => {
                        o["imageBitmap"] = imageBitmap;
