globalThis.WebSdkWrapper = (function() {
  ...
  */
    ...
    let supportedNetworks = [{
        ...
        name: "CrazyGames",
        get sdk() {
            if (!sdkContext.crazysdk && globalThis.CrazyGames && globalThis.CrazyGames.CrazySDK) sdkContext.crazysdk = globalThis.CrazyGames.CrazySDK.getInstance();
        ...
        implementation: {
            ...
            setUpEventListeners() {
                ...
                sdk.addEventListener("adFinished", () => {
                    if (sdkContext.lastRequestedAd === "interstitial") dispatch("interstitialEnd", true);
                ...
                sdk.addEventListener("adError", () => {
                    if (sdkContext.lastRequestedAd === "interstitial") dispatch("interstitialEnd", false);
        ...
        implementation: {
            ...
            setUpEventListeners() {
                ...
                listen("rewarded", () => {
                    sdkContext.lastRequestedAd = "rewarded";
                    dispatch("adStarted", sdkContext.lastRequestedAd);
                    sdk.showAd("rewarded").then((response) => {
    ...
    const Wrapper = {
            ...
            return new Promise(async (resolve) => {
                ...
                if (currentSdk) {
                    ...
                    if (currentSdk.enableOnlyInProduction && debug) {
                        ...
                        if (currentSdk.implementation.preInit) await currentSdk.implementation.preInit(debug, data);
                        if (currentSdk.scriptSrc) {
                            addScript(currentSdk.scriptSrc, currentSdk.name + "-jssdk", async () => {
                                sdk = currentSdk.sdk;
                                currentSdk.implementation.setUpEventListeners();
                                if (currentSdk.implementation.init) await currentSdk.implementation.init(debug, data);
                                resolve();
                            });
                        } else {
                            resolve();
        ...
        hasAdblock() {
            if (currentSdk && currentSdk.implementation.hasAdblock) return currentSdk.implementation.hasAdblock();
            return false;
        },
        loadingStart() {
